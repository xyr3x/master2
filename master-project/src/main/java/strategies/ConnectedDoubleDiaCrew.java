...
public class ConnectedDoubleDiaCrew extends ConnectedFireFighterCrew{
	...
	public void initialize() {
		...
		if(size % 4 == 0) {
			...
			//dia ready, split up dia, half of the fighters move south, half move up
			//first half
			for(int i = 0; i < ((crew.size() / 2) + 1); i++) {
				for (int j = 5; j < Main.TimeInterval; j++) {
					crew.get(i).setChainIndex(j, 1);
				}
